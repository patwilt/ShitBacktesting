...
## Important Design Note: Dynamic Strategy Labels

All titles, chart legends, pie chart labels, and metrics must be derived from the CSV data — **never hardcoded**. The backtest suite supports arbitrary weight combinations (e.g. IVV/UPRO/Gold, MSCI/UPRO/Gold, SPY/UPRO/Gold) and the dashboard must adapt automatically.

`BacktestData` includes an `allocations: dict[str, dict[str, float]]` field parsed from strategy names. Strategy names follow the pattern `"Hybrid SPY (70/15/15)"` or `"Dull S&P 500"`. The parser extracts asset labels and weight percentages from these names to populate allocation pie charts and labels dynamically.
...
def test_parse_strategy_allocation_hybrid_spy():
    from utils.csv_loader import parse_strategy_allocation
...
# ── Asset allocation (dynamically parsed from strategy names) ─────────────────
st.divider()
